...
const veryfyEmail = async(req,res,next)=>{
    try {
        ...
        if(foundUser.isVerified) return res.status(200).json({success:true,message:'Email is already verified'});

        if(foundUser.otp !== Number(otp)) return res.status(400).json({success:false,message:'Invalid OTP'});
